...
export function buildPlayCommand(kind, config, options) {
  ...
  const inner = [
    "Start-Process",
    "-FilePath " + psQuote(resolvePython(config).path),
...
export function playKind(kind, config, log) {
  ...
  try {
    ...
    if (!pythonAvailable(config)) {
      say("play skipped: python not found: " + String(config.pythonPath) + " (resolved=" + resolvePython(config).path + ")");
...
/**
 * 解析一个**真实可用**的 python 解释器（开源必备，2026-09-16）。
 *
 * 背景：默认值写死本机路径 (`E:\Python311\python.exe`) 会把开源版绑死在某台机器上；
 * 但直接改成 `"python"` 又会踩到 Windows 的 App Execution Alias 存根
 * （`C:\...\WindowsApps\python.exe`，执行只返回 9009 并提示去应用商店）——
 * 本机实测 `python`/`python3` 都是这种存根，真正可用的是 `where python` 里的 `E:\Python311\python.exe`。
 *
 * 候选顺序（用 `sys.executable` 取真实路径，天然跳过存根）：
 *   1) config.pythonPath —— 用户显式配置，优先级最高
 *   2) `where python` 的每一项，跳过 WindowsApps 存根
 *   3) `python` / `python3`（PATH 里的真解释器）
 *   4) `py`（Windows Python Launcher，最后手段）
 * 结果缓存，避免每次播放都 spawn 一次探测。
 */
let resolvedPythonCache = null;
export function resolvePython(config, options) {
  const opts = options || {};
  const configured = String((config && config.pythonPath) || "").trim();
  if (!opts.noCache && resolvedPythonCache && resolvedPythonCache.key === configured) {
    return resolvedPythonCache.value;
  }
  const probe = function (command) {
    try {
      const run = spawnSync(command, ["-c", "import sys;print(sys.executable)"], {
        encoding: "utf8",
        windowsHide: true,
        timeout: 6000,
      });
      const real = String(run.stdout || "").trim();
      if (run.status === 0 && real.length > 0 && existsSync(real)) return real;
    } catch (_) {
      /* 试下一个候选 */
    }
    return null;
  ...
  try {
    const where = spawnSync("where", ["python"], { encoding: "utf8", windowsHide: true, timeout: 6000 });
    for (const line of String(where.stdout || "").split(/\r?\n/u)) {
      const candidate = line.trim();
      if (candidate.length === 0 || /WindowsApps/iu.test(candidate)) continue;
  ...
  let found = null;
  for (const candidate of candidates) {
    const real = probe(candidate);
    if (real) {
      found = { path: real, source: candidate };
      break;
    }
  }
  const value = found || { path: configured || "python", source: "fallback" };
  resolvedPythonCache = { key: configured, value };
  return value;
}

/**
 * config 里的 pythonPath 是否真的可用。
 * 注意 config.pythonPath 默认是 **PATH 名**（"python"），`existsSync("python")` 恒为 false，
 * 所以必须看解析结果（resolvePython 会跳过 WindowsApps 存根并给出真实路径）。
 */
export function pythonAvailable(config) {
  const resolved = resolvePython(config);
  return resolved.source !== "fallback" && existsSync(resolved.path);
}

/**
 * 预热的命令行参数。
 *
 * 🔴 默认**不预热**（config.prewarmMs = 0）：2026-09-16 用户实测发现，预热会让蓝牙链路在播报前
 * 多开关一次音频端点，从而**打断正在播放的音乐**；去掉预热后立即恢复正常。
 * 只有显式把 prewarmMs 配成 >0 时才传 `--prewarm-ms N`（给确实需要唤醒链路的设备用）。
 */
export function prewarmArgsFor(config) {
  const ms = Number(config && config.prewarmMs);
  return ms > 0 ? ["--prewarm-ms", String(Math.round(ms))] : ["--no-prewarm"];
}
...
export function tryPlayViaWav(file, config, log) {
  ...
  if (!pythonAvailable(config)) return { ok: false, reason: "python-missing" };
  try {
    spawnLauncher(command);
